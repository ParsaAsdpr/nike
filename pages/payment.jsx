    ...
    <Layout title="نحوه پرداخت">
      ...
      >
        ...
        {["درگاه بانکی", "پرداخت درب منزل"].map((payment, index) => (
          <div
            key={payment}
            className="mb-4 py-4 px-7 border border-stone-800 bg-[#131313] text-xl rounded-md flex flex-row items-center gap-1"
          >
            <input
              name="paymentMethod"
            ...
            />
            <label
              className="w-7 p-1.5 h-7 border border-stone-800 bg-stone-900 rounded-full cursor-pointer"
              htmlFor={`option${index}`}
            >
              <div
                className={`w-full h-full bg-green-500 rounded-full transition ${
                  isClicked == index ? "scale-100" : "scale-0"
                }`}
              ></div>
            </label>
            &nbsp;
            <label
              className={`p-2 cursor-pointer transition ${
                isClicked == index ? "text-green-500" : "text-stone-200"
              }`}
              htmlFor={`option${index}`}
            >
              {payment}
